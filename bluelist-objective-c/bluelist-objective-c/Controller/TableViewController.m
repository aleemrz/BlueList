...
#import "LeftBarViewController/LeftBarViewController.h"
#import "User.h"


@interface TableViewController () <UITextFieldDelegate, CDTReplicatorDelegate, CLLocationManagerDelegate, CDRTranslucentSideBarDelegate>
...
- (void)viewDidLoad {
    [super viewDidLoad];
    NSLog(@"userIdentity: %@", [IMFAuthorizationManager sharedInstance].userIdentity);
    debug = YES;
...
-(void)viewDidAppear:(BOOL)animated{
    [self.logger logTraceWithMessages:@"viewDidAppear called"];
    [self pullItems];
}

-(void)viewDidDisappear:(BOOL)animated{
    [self.logger logTraceWithMessages:@"viewDidDisappear called"];
    [self pushItems];
}
...
- (IBAction)settingButtonPressed:(id)sender {
    UIViewController *controller = [self.storyboard instantiateViewControllerWithIdentifier:@"LeftBarViewController"];
    
//    LeftBarViewController *controller = [LeftBarViewController new];
    
    self.sideBar = [[CDRTranslucentSideBar alloc] initWithDirectionFromRight:NO];
    self.sideBar.view.frame = CGRectMake(0, 0, 300, self.view.frame.size.height);
    controller.view.frame = self.sideBar.view.frame;
    self.sideBar.delegate = self;
    self.sideBar.translucentStyle = UIBarStyleDefault;
    // Set ContentView in SideBar
    [self.sideBar setContentViewInSideBar:controller.view];
...
-(void)imageTapped :(id) sender
{
    ...
    [self changePriorityForCell:[self.tableView cellForRowAtIndexPath:tappedIndexPath]];
    ...
    if( indexPath.section == 0) {
        ...
        for (UIView *view in [cell.contentView subviews]) {
            if([view isKindOfClass:[UITextField class]]){
                //deprecated.. this text field was removed.
...
//    NSDictionary* geoDict = [self geoRepresentation];
    ...
    item.body = @{
                  ...
                  PROPERTIES_FIELD : @{
                          ...
                          USER_ID : [IMFAuthorizationManager sharedInstance].userIdentity
